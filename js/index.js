...
const sortTable = (index, type) => {
    const compare = (a, b) => {
        const rowDataA =  a.cells[index].innerHTML;
        const rowDataB = b.cells[index].innerHTML;
        switch (type) {
            case 'num':
...
// Слушатель событий клика на заголовках таблицы
...
class TableRow  {
    constructor(id, firstName, lastName, descr, eyeColor, parentElem) {
        this.id = id;
    ...
    render() {
        ...
        tableRow.innerHTML = `
            <td data-type="num">${this.id}</td>
            <td data-type="text">${this.firstName}</td>
            <td data-type="text">${this.lastName}</td>
            <td data-type="text">
            <p class="about">
                ${this.descr}
            </p>
            </td>
            <td data-type="text">${this.eyeColor}</td>
        `

        this.parentElem.append(tableRow);
    }
}
...
const data = [
  ...
  data.forEach((obj, index) => {
      new TableRow(index + 1, obj.name.firstName, obj.name.lastName, obj.about, obj.eyeColor, tbody).render()
  })
